...
function criarBotaoIA(caixa) {
  ...
  btn.onclick = async (e) => {
    ...
    const { texto, tipo } = encontrarTextoRelacionado(caixa);
    ...
    const res = await fetch(
      'https://n8n-n8n.dodhyu.easypanel.host/webhook/comentario-linkedin',
      {
        ...
        body: JSON.stringify({ texto, tipoDetectado: tipo }),
      }
    );
...
function encontrarTextoRelacionado(caixa) {
  ...
  let el = caixa.closest('.comments-comment-item');
  if (el) {
    ...
    const tipo = el.closest('.comments-comment-item__nested')
      ? 'subcomentario'
      : 'resposta';
  ...
  let post = caixa.closest('[data-id]')              // feed
          || caixa.closest('article[data-urn]')     // perfil/newsletter
          || caixa.closest('article');             // fallback

  const full = post?.innerText || '';
  let texto = full;

  // Se existir marcador de newsletter, remove até após o nome
  const marker = '\nSugestões\n';
  const idx = full.indexOf(marker);
  if (idx !== -1) {
    // pula duas linhas: 'Sugestões' e o próprio nome
    const after = full.slice(idx + marker.length).split('\n').slice(1).join('\n');
    texto = after;
  }
